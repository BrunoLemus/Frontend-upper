...
export default {
  ...
  data() {
    return {
      ...
      factura: "",
      numTarimas: 0, // Corregido de numCajas
      tipoEmbalaje: null,
      claveProducto: "",
  ...
  methods: {
    ...
    // ... Tu función guardarDatos se mantiene igual (usa 'requiereDimensiones') ...
    async guardarDatos() {
        if (
            !this.factura ||
            !this.tipoEmbalaje ||
        ...
        try {
            ...
            const response = await fetch(
                `https://backend-upper.onrender.com/tarimas/?sede=${sede}&role=${role}`,
                {
                    method: "POST",
                    headers: {
    ...
    // ... Tu función generateQR se mantiene igual ...
